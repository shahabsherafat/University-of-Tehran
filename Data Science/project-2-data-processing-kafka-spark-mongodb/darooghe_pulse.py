...
def generate_transaction_event(is_historical=False, timestamp_override=None):
    event_time = (
        timestamp_override if timestamp_override else datetime.datetime.now()
    )
...
def produce_historical_events(producer, topic, count=200000):
    logging.info(f"Producing {count} historical events...")
    now_time = datetime.datetime.now()
    start_time = datetime.datetime.now() - timedelta(days=7)
    for _ in range(count):
        event_time = generate_random_datetime(start_time, now_time)
        event = generate_transaction_event(timestamp_override=event_time,is_historical=True)
        producer.produce(
            topic,
...
if __name__ == "__main__":
    ...
    # if not skip_initial:
    print(1)
    produce_historical_events(producer, topic, count=200000)
